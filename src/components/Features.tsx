      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 sm:mb-20 gap-8">
          <div className="max-w-2xl">
            <p className="text-amber-500 font-mono text-xs sm:text-sm uppercase tracking-[0.3em] font-bold mb-4">Core Philosophy</p>
        ...
        <div className="grid md:grid-cols-2 gap-px bg-white/5 border border-white/5 rounded-3xl overflow-hidden">
          {pillars.map((pillar, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="p-8 sm:p-12 bg-[#0A0A0A] hover:bg-zinc-900 transition-colors group relative"
            >
              <div className="flex justify-between items-start mb-6 sm:mb-8">
                <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br ${pillar.color} flex items-center justify-center shadow-2xl`}>
                  <pillar.icon className="text-white w-6 h-6 sm:w-8 sm:h-8" />
                </div>
                <span className="text-4xl sm:text-6xl font-black text-white/5 font-sans group-hover:text-white/10 transition-colors">{pillar.id}</span>
              </div>
              
              <h3 className="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight mb-4 group-hover:text-amber-500 transition-colors">
                {pillar.title}
              </h3>
