...
import { Users, Building2, GraduationCap, Heart, CheckCircle2, Shield } from 'lucide-react';
...
export default function Clubs({ onJoinClick }: { onJoinClick: () => void }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/content/clubs`)
      .then(res => res.json())
      .then(data => setClubs(data))
      .catch(err => console.error("Failed fetching clubs", err));
  }, []);
  ...
  return (
    <section id="clubs" className="py-32 bg-[#050505] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-8">
          {clubs.map((club, idx) => (
            ...
            >
              ...
              <div className={`w-20 h-20 rounded-3xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center mb-8 overflow-hidden`}>
                {club.image ? (
                  <img src={club.image?.startsWith('/uploads') ? `${API_URL}${club.image}` : club.image} alt={club.name} className="w-full h-full object-cover" />
                ) : (
                  <Shield className={`text-amber-500 w-10 h-10`} />
                )}
              </div>
              ...
              <div className="w-full space-y-3 mb-10">
                <div className="flex items-center justify-center space-x-2 text-xs font-bold text-gray-400 uppercase tracking-widest text-center">
        ...
        {clubs.length === 0 && (
          <div className="text-center py-20 text-gray-500 italic">No clubs are currently active.</div>
        )}
      </div>
    </section>
  );
